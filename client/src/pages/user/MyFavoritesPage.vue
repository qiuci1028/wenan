<template>
  <div class="py-8">
    <div class="max-w-page mx-auto px-6">
      <h1 class="text-2xl font-semibold mb-6">我的收藏</h1>

      <div v-if="loading" class="flex justify-center py-16">
        <el-icon class="is-loading text-3xl text-secondary"><Loading /></el-icon>
      </div>

      <div v-else-if="wenanStore.myFavorites.length === 0" class="empty-state">
        <div class="empty-state-icon">⭐</div>
        <p class="text-lg mb-2">还没有收藏过文案</p>
...
onMounted(async () => {
  try {
    await wenanStore.fetchMyFavorites()
  } finally {
    loading.value = false
  }
})
